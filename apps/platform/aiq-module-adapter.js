...
async function getContext(){const {data,error}=await sb.rpc('my_platform_context');if(error){console.warn('[AIQ] context read failed',error.message);return null}return data}
...
function dispatch(ctx){window.AIQ_CONTEXT=ctx;window.dispatchEvent(new CustomEvent('aiq:context',{detail:ctx}));document.dispatchEvent(new CustomEvent('aiq:context',{detail:ctx}));}
...
window.ApplianceIQ={supabase:sb,moduleKey,getContext,setContext,focus:(type,id,label,context={})=>setContext({entity_type:type,entity_id:id,entity_label:label,context}),clearFocus:()=>setContext({entity_type:null,entity_id:null,entity_label:null,context:{}})};
