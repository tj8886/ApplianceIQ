...
create unique index if not exists ai_coaching_strategy_perf_uniq on public.ai_coaching_strategy_performance(organization_id,metric_key,coalesce(skill_id,'00000000-0000-0000-0000-000000000000'::uuid),strategy_key,difficulty_level);

create table if not exists public.ai_adaptive_coaching_decisions (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  user_id uuid not null,
  intervention_id uuid not null references public.ai_coaching_interventions(id) on delete cascade,
...
create index if not exists ai_adaptive_profiles_org_user_idx on public.ai_adaptive_coaching_profiles(organization_id,user_id);
create index if not exists ai_strategy_performance_org_metric_idx on public.ai_coaching_strategy_performance(organization_id,metric_key,posterior_success desc);
create index if not exists ai_adaptive_decisions_org_user_idx on public.ai_adaptive_coaching_decisions(organization_id,user_id,created_at desc);
...
revoke all on public.ai_adaptive_coaching_profiles, public.ai_coaching_strategy_performance, public.ai_adaptive_coaching_decisions from anon, authenticated;
grant all on public.ai_adaptive_coaching_profiles, public.ai_coaching_strategy_performance, public.ai_adaptive_coaching_decisions to service_role;
...
revoke all on function public.phase5_refresh_profile(uuid,uuid) from public,anon;
revoke all on function public.phase5_select_strategy(uuid,uuid,text,uuid) from public,anon;
revoke all on function public.phase5_apply_adaptation(uuid) from public,anon;
revoke all on function public.phase5_generate_adaptive_coaching(uuid,uuid,date) from public,anon;
revoke all on function public.phase5_rep_plan(uuid,uuid) from public,anon;
revoke all on function public.phase5_manager_dashboard(uuid) from public,anon;
grant execute on function public.phase5_refresh_profile(uuid,uuid),public.phase5_select_strategy(uuid,uuid,text,uuid),public.phase5_apply_adaptation(uuid),public.phase5_generate_adaptive_coaching(uuid,uuid,date),public.phase5_rep_plan(uuid,uuid),public.phase5_manager_dashboard(uuid) to authenticated,service_role;
revoke all on function public.phase5_learn_from_evaluation(uuid),public.phase5_evaluation_learning_trigger() from public,anon,authenticated;
grant execute on function public.phase5_learn_from_evaluation(uuid),public.phase5_evaluation_learning_trigger() to service_role;
